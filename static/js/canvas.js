...
const DEFAULT_AGENT_INSTRUCTION = 'You are a focused canvas agent. Follow the custom instructions and use connected text or image context when available. Return only the result needed by downstream nodes.';
...
function defaultApiProviders(){
    return [{id:'comfly', name:'Comfly', base_url:'', protocol:'openai', enabled:true, image_models:imageModels, chat_models:chatModels, video_models:videoModels, has_key:false, key_preview:''}];
}
...
function providerImageModels(providerId){
    const imageProvider = apiProviders.find(p => p.id === providerId);
    // 不走 providerById（会 fallback 到第一个 provider，造成串台），直接查精确匹配
    const provider = apiProviders.find(p => p.id === providerId);
    return uniqueModels(imageProvider?.image_models || []);
}
...
function providerVideoModels(providerId){
    const providerPool = apiProviders.length ? apiProviders : defaultApiProviders();
    const videoProvider = providerPool.find(p => p.id === providerId);
    if(!videoProvider) return [];
    // 不走 providerById（会 fallback 到第一个 provider，造成串台），直接查精确匹配
    const models = videoProvider?.video_models || [];
...
async function loadConfig(){
    ...
    try {
        ...
        videoModels = cfg.video_models?.length ? cfg.video_models : [];
...
function openSmartCanvasPage(id){
    if(!id) return;
    window.location.href = `/static/smart-canvas.html?id=${encodeURIComponent(id)}&v=2026.05.28.2`;
}
...
function addVideoNode(point){
    ...
    const providerId = videoApiProviders()[0]?.id || '';
    const models = providerVideoModels(providerId);
    return addNode({
        id:uid('vid'),
        type:'video',
...
function linkCreateOptions(state){
    ...
    if(state.originKind === 'out'){
        if(['image','prompt','loop','group','promptGroup','llm','agent'].includes(node.type)){
            return [
    ...
    if(CANVAS_GENERATOR_TYPES.includes(node.type) || ['llm','agent'].includes(node.type)){
        return [
...
function openGeneratorNodeMenu(nodeId, clientX, clientY){
    const node = nodes.find(n => n.id === nodeId);
    if(!node || (!CANVAS_GENERATOR_TYPES.includes(node.type) && !['llm','agent'].includes(node.type))) return false;
    ...
    const outputOptions = ['llm','agent'].includes(node.type) ? linkCreateOptions({originId:nodeId, originKind:'out', point}) : [
...
function menuAdd(type){
    ...
    if(type === 'agent') addAgentNode(menuPoint);
...
function renderNode(node){
    ...
    el.oncontextmenu = e => {
        if(!CANVAS_GENERATOR_TYPES.includes(node.type) && !['agent','llm','output'].includes(node.type)) return;
        e.preventDefault();
        e.stopPropagation();
        if(node.type === 'output') openOutputNodeMenu(node.id, e.clientX, e.clientY);
        else openGeneratorNodeMenu(node.id, e.clientX, e.clientY);
    };
    const title = node.type === 'image' ? 'Image' : node.type === 'prompt' ? 'Prompt' : node.type === 'loop' ? tr('canvas.loopNode') : node.type === 'promptGroup' ? 'Prompts' : node.type === 'group' ? 'Group' : node.type === 'output' ? 'Output' : node.type === 'llm' ? 'LLM' : node.type === 'agent' ? (node.agentName || 'Agent') : node.type === 'comfy' ? 'ComfyUI' : node.type === 'msgen' ? tr('canvas.modelscopeGenerate') : node.type === 'video' ? tr('canvas.videoGenerateNode') : tr('canvas.apiGenerate');
    const displayTitle = node.type === 'image' && node.url ? nodeTitleForMedia(node) : title;
    // 失败徽章只在一键运行模式中显示，单节点失败已通过 alert 提示
    const showStatus = ['generator','msgen','comfy','llm','agent'].includes(node.type) && node.runStatus
    ...
    if(node.type === 'agent') body.appendChild(renderAgentBody(node));
    ...
    const canInput = ['generator','comfy','output','llm','agent','msgen','video'].includes(node.type) || (node.type === 'loop' && (node.imageInput || node.showPrompt));
    const canOutput = ['image','prompt','loop','group','promptGroup','generator','comfy','llm','agent','msgen','video'].includes(node.type);
...
function defaultNodeSize(type){
    ...
    if(type === 'agent') return {w:420, h:640};
...
function loopInputPromptItems(node){
    ...
    try {
        ...
        else if(n.type === 'llm' || n.type === 'agent') text = n.outputText || '';
...
function renderAgentBody(node){
    const wrap = document.createElement('div');
    wrap.className = 'llm-body agent-body';
    node.agentName = node.agentName || 'Agent';
    if(node.agentInstruction == null) node.agentInstruction = node.systemPrompt || DEFAULT_AGENT_INSTRUCTION;
    node.llmProvider = resolveChatProviderId(node.llmProvider || 'comfly');
    const llmProv = node.llmProvider;
    if(llmProv === 'modelscope') node.model = node.llmMsModel || node.model;
    const models = providerChatModels(llmProv);
    if(models.length && !models.includes(node.model)) node.model = models[0] || node.model;
    const modelOpts = chatModelOptions(node.model, llmProv);
    const imgs = llmInputImages(node);
    const connectedInput = llmInputText(node);
    const isReadonly = connectedInput.length > 0;
    ...
    const inputHeight = Math.max(70, node.llmInputHeight || 110);
    const outputHeight = Math.max(70, node.llmOutputHeight || 150);
    const inputPlaceholder = langIsEn() ? 'Type a task, or connect Prompt/Image/Output nodes.' : '输入任务，或连接提示词/图片/输出节点。';
    const instructionPlaceholder = langIsEn()
        ? 'Define this agent role, rules, style, tools to simulate, and output format.'
        : '定义这个 Agent 的角色、规则、风格、模拟工具和输出格式。';
    const imgBadge = imgs.length ? `<div class="agent-context-badge"><i data-lucide="image" class="w-3 h-3"></i>${imgs.length} image${imgs.length > 1 ? 's' : ''} connected. Use a vision model if needed.</div>` : '';
    wrap.innerHTML = `
        <div class="llm-row agent-top-row">
            <input class="agent-name-input" value="${escapeAttr(node.agentName)}" maxlength="40" placeholder="Agent">
        </div>
        <div class="llm-row">
            <select class="select-lite llm-provider-select" style="flex:1">${chatProviderOptions(llmProv)}</select>
            <select class="select-lite llm-model">${modelOpts}</select>
        ...
        ${imgBadge}
        <div class="llm-pane-label">Instructions</div>
        <textarea class="llm-system agent-instruction" placeholder="${escapeAttr(instructionPlaceholder)}">${escapeHtml(node.agentInstruction || '')}</textarea>
        <div class="llm-pane-label">Input${isReadonly ? ' <span style="font-size:9px;opacity:.5;font-weight:600;text-transform:none;letter-spacing:0">(from links)</span>' : ''}</div>
        <textarea class="llm-input-area llm-input-output" style="height:${inputHeight}px; flex:0 0 ${inputHeight}px;" ${isReadonly ? 'readonly' : ''} placeholder="${escapeAttr(inputPlaceholder)}">${escapeHtml(inputValue)}</textarea>
        <div class="llm-pane-resizer" title="${tr('canvas.resizePanes')}"></div>
        <div class="llm-pane-label">Output</div>
        <div class="llm-output-wrap" style="height:${outputHeight}px; flex:0 0 ${outputHeight}px;">
            <button class="llm-copy-btn llm-output-copy" type="button" title="Copy"><i data-lucide="copy" class="w-3.5 h-3.5"></i></button>
            <div class="llm-output llm-result-output">${escapeHtml(node.outputText || tr('canvas.llmOutputEmpty'))}</div>
        </div>
        <div class="gen-run-row mt-2">
            <button class="llm-run ${node.running ? 'running' : ''}" ${node.running ? 'disabled' : ''}><i data-lucide="play" class="w-4 h-4"></i>${node.running ? tr('canvas.running') : 'Run Agent'}</button>
            ${cascadeBtnHtml(node)}
        </div>
        ${retryBarHtml(node)}
    `;
    const nameInput = wrap.querySelector('.agent-name-input');
    const providerSelect = wrap.querySelector('.llm-provider-select');
    const modelSelect = wrap.querySelector('.llm-model');
    providerSelect.value = llmProv;
    modelSelect.value = resolveChatModel(node.model, llmProv);
    [nameInput, providerSelect, modelSelect].forEach(input => {
        input.onmousedown = e => e.stopPropagation();
        input.onclick = e => e.stopPropagation();
    });
    nameInput.oninput = e => {
        node.agentName = e.target.value || 'Agent';
        const titleEl = nodesEl.querySelector(`.node[data-id="${CSS.escape(node.id)}"] .node-title`);
        if(titleEl) titleEl.textContent = node.agentName || 'Agent';
        scheduleSave();
    };
    providerSelect.onchange = e => {
        e.stopPropagation();
    ...
    const instructionEl = wrap.querySelector('.agent-instruction');
    instructionEl.oninput = e => {
        node.agentInstruction = e.target.value;
        node.systemPrompt = e.target.value;
        scheduleSave();
    };
    bindScrollableText(instructionEl);
    const inputEl = wrap.querySelector('.llm-input-output');
    bindScrollableText(inputEl);
    if(!isReadonly){
        inputEl.oninput = e => { node.userInput = e.target.value; scheduleSave(); };
    }
    bindScrollableText(wrap.querySelector('.llm-result-output'));
    wrap.querySelector('.llm-pane-resizer').onmousedown = e => startLLMPaneResize(e, node);
    wrap.querySelector('.llm-run').onclick = e => { e.stopPropagation(); runAgentNode(node.id); };
    bindCascadeButtons(wrap, node.id);
    const copyBtn = wrap.querySelector('.llm-output-copy');
    copyBtn.onmousedown = e => e.stopPropagation();
    copyBtn.onclick = async e => {
        e.stopPropagation();
        const text = node.outputText || '';
...
function llmInputText(node){
    return connections.filter(c => c.to === node.id).map(c => nodes.find(n => n.id === c.from)).filter(Boolean).map(n => {
        ...
        if(n.type === 'llm' || n.type === 'agent') return n.outputText || '';
        return '';
    }).filter(Boolean).join('\n\n');
}
function llmInputImages(node){
    ...
    connections.filter(c => c.to === node.id).map(c => nodes.find(n => n.id === c.from)).filter(Boolean).forEach(n => {
        ...
        if(CANVAS_IMAGE_OUTPUT_TYPES.includes(n.type)){
            generatedImageRefs(n).forEach(ref => { if(ref.url) urls.push(ref.url); });
        }
        if(n.type === 'group'){
...
function renderVideoBody(node){
    ...
    if(!node.model || !availableVideoModels.includes(node.model)) node.model = availableVideoModels[0] || '';
    wrap.innerHTML = `
        <div class="prompt-list mb-3"></div>
...
function generatorSources(gen){
    return connections.filter(c => c.to === gen.id).map(c => nodes.find(n => n.id === c.from)).filter(Boolean).map(n => {
        ...
        if(n.type === 'agent' && n.outputText) return {id:n.id, type:'agent', label:(n.agentName || n.outputText || 'Agent').slice(0, 32), refs:[], prompt:n.outputText || ''};
        return null;
    }).flat().filter(Boolean);
}
...
async function runVideoNode(nodeId, opts={}){
    ...
    if(!node.model){ alert(tr('canvas.noModelsHint') || '暂无模型，请到 API 设置添加'); return; }
    let out = outputForNode(node, 460);
...
function agentSystemPrompt(node){
    const name = String(node?.agentName || 'Agent').trim() || 'Agent';
    const instruction = String(node?.agentInstruction || node?.systemPrompt || DEFAULT_AGENT_INSTRUCTION).trim() || DEFAULT_AGENT_INSTRUCTION;
    return `You are ${name}.\n\n${instruction}`;
}
async function callCanvasLLM(node, message, messages=[]){
    ...
    const result = await fetch('/api/canvas-llm', {
        ...
        body:JSON.stringify({
            ...
            provider: llmProv,
            system_prompt:node.type === 'agent' ? agentSystemPrompt(node) : (node.systemPrompt || 'You are a helpful assistant.'),
            messages,
            images,
        })
    }).then(async r => {
...
async function runAgentNode(nodeId, opts={}){
    const node = nodes.find(n => n.id === nodeId);
    if(!node || (node.running && !opts.cascade)) return;
    const input = llmInputText(node) || node.userInput || '';
    const message = input || 'Follow your agent instructions and produce the result for the canvas.';
    if(!opts.cascade){ node.running = true; refreshNodes([node.id]); }
    try {
        node.outputText = await callCanvasLLM(node, message, []);
        if(!opts.cascade) node.running = false;
        node.runStatus = 'done'; node.runError = '';
        refreshNodes([node.id]);
        scheduleSave();
        ...
        refreshNodes([node.id]);
        if(opts.cascade) throw err;
        alert(err.message || 'Agent run failed');
    }
}
// 判断是不是「链尾」节点：没有下游生成节点（直接相连或经 Output 中转都算）
function isTerminalGenerator(nodeId){
    const GEN_TYPES = ['generator','msgen','comfy','llm','agent','video'];
...
function runCascadeNodeByType(node, opts={}){
    ...
    if(node.type === 'agent') return runAgentNode(node.id, runOpts);
...
function canvasRunTypes(){
    return ['generator','msgen','comfy','llm','agent','video'];
}
...
function computeConnectedWorkflowOrder(anchorId){
    ...
    const runTypes = canvasRunTypes();
    const execTypes = runTypes.filter(t => !['llm', 'agent'].includes(t));
    ...
    const visit = id => {
        ...
        if(connected.has(id) && execTypes.includes(nodes.find(n => n.id === id)?.type)) order.push(id);
    };
    nodes.filter(n => connected.has(n.id) && execTypes.includes(n.type)).forEach(n => visit(n.id));
    return order;
}
async function runCanvasGenerate(nodeId){
    const node = nodes.find(n => n.id === nodeId);
    if(!node || node.running || cascadeRunningIds.has(nodeId)) return;
    // Single run mode: execute only the target node itself.
...
function computeCascadeOrder(targetId){
    ...
    const INPUT_ONLY_TYPES = new Set(['llm', 'agent']);
    function dfs(id){
        if(visited.has(id)) return;
        visited.add(id);
        const node = nodes.find(n => n.id === id);
        if(!node) return;
        if(INPUT_ONLY_TYPES.has(node.type)) return;
        // 找该节点的上游
        connections.filter(c => c.to === id).forEach(c => {
            const from = nodes.find(n => n.id === c.from);
            if(!from) return;
            if(INPUT_ONLY_TYPES.has(from.type)) return;
            if(GEN_TYPES.includes(from.type)){
                ...
                connections.filter(cc => cc.to === from.id).forEach(cc => {
                    ...
                    if(ff && GEN_TYPES.includes(ff.type) && !INPUT_ONLY_TYPES.has(ff.type)) dfs(ff.id);
                });
            }
        });
...
async function runOneCascadePass(order, options={}){
    const execOrder = order.filter(id => {
        const n = nodes.find(x => x.id === id);
        return n && !['llm', 'agent'].includes(n.type);
    });
    ...
    for(let i = 0; i < execOrder.length; i++){
        ...
        try {
            ...
            else if(node.type === 'agent') await runAgentNode(id, {cascade:true});
...
function canConnect(fromId, toId){
    ...
    if(CANVAS_GENERATOR_TYPES.includes(from.type)){
        ...
        if(CANVAS_IMAGE_OUTPUT_TYPES.includes(from.type) && (CANVAS_GENERATOR_TYPES.includes(to.type) || ['llm','agent'].includes(to.type))){
    ...
    if(to.type === 'loop'){
        ...
        const allowPrompt = Boolean(to.showPrompt) && ['prompt','promptGroup','loop','llm','agent'].includes(from.type);
        return allowImage || allowPrompt;
    }
    if(to.type === 'llm' || to.type === 'agent') return ['prompt','loop','promptGroup','llm','agent','image','group','output'].includes(from.type) && !wouldCreateGeneratorCycle(fromId, toId);
    if(from.type === 'llm' || from.type === 'agent') return CANVAS_GENERATOR_TYPES.includes(to.type) && !wouldCreateGeneratorCycle(fromId, toId);
    return CANVAS_GENERATOR_TYPES.includes(to.type) && ['image','prompt','loop','group','promptGroup','output','llm','agent'].includes(from.type);
}
