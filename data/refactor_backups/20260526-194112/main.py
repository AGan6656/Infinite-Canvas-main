...
def ensure_runtime_config_files():
    """首次运行时提前创建配置目录，避免第一次保?API Key 时才创建目录/文件?""
    try:
...
def friendly_validation_error(errors):
    ...
    for err in errors or []:
        ...
        if "max_length" in err_type or "at most" in msg:
            parts.append(f"{label}过长：当前内容超过后端上?{limit} 个字符。请拆分为多个提示词节点，或先用 LLM 节点压缩后再生成?)
...
VIDEO_MODELS = model_list("VIDEO_MODELS", "veo3-fast", [
    # —?Veo 系列 —?    "veo2", "veo2-fast", "veo2-pro",
    "veo3", "veo3-fast", "veo3-pro",
    "veo3.1", "veo3.1-fast", "veo3.1-quality", "veo3.1-lite",
    # —?Sora —?    "sora-2", "sora-2-pro",
    # —?阿里 通义万相 —?    "wan2.6-t2v", "wan2.6-i2v",
    "wan2.5-t2v-preview", "wan2.5-i2v-preview",
    "wan2.2-t2v-plus", "wan2.2-i2v-plus", "wan2.2-i2v-flash",
    # —?火山 豆包 Seedance —?    "doubao-seedance-2-0-260128",
    "doubao-seedance-2-0-fast-260128",
    "doubao-seedance-1-5-pro-251215",
    "doubao-seedance-1-0-pro-250528",
    "doubao-seedance-1-0-lite-t2v-250428",
...
def default_api_providers():
    # 只保?ModelScope 为强制默认平台，其他平台均可自定义增?    return [
        {
            "id": "modelscope",
            "name": "ModelScope",
...
def merge_default_api_providers(providers):
    ...
    # 强制保留独立入口平台（不再强?comfly?    ms_default = next((d for d in default_api_providers() if d["id"] == "modelscope"), None)
    if ms_default:
...
def normalize_endpoint_override(value, label):
    ...
    if len(endpoint) > 300 or re.search(r"\s", endpoint):
        raise HTTPException(status_code=400, detail=f"{label} 不合法，请填写类?/v1/images/edits 的路?)
    if re.match(r"^https?://", endpoint, re.I):
        return endpoint.rstrip("/")
    if not endpoint.startswith("/"):
        raise HTTPException(status_code=400, detail=f"{label} 需要以 /v1/... 开头，或填写完?http(s) 地址")
    return endpoint
...
def normalize_provider(item):
    ...
    if base_url and not re.match(r"^https?://", base_url):
        raise HTTPException(status_code=400, detail=f"{name} ?Base URL 需要以 http:// ?https:// 开?)
    protocol = str(item.get("protocol") or "openai").strip().lower()
    if protocol not in SUPPORTED_PROVIDER_PROTOCOLS:
        protocol = "openai"
    image_generation_endpoint = normalize_endpoint_override(item.get("image_generation_endpoint"), "文生图端?)
    image_edit_endpoint = normalize_endpoint_override(item.get("image_edit_endpoint"), "图生?编辑端口")
    return {
        "id": provider_id,
        "name": name,
        "base_url": base_url,
...
def get_primary_provider_id(providers=None):
    """返回当前首?provider ?id；优?primary=True 的，否则取第一个非 modelscope 的，再次取第一个?""
...
def get_api_provider(provider_id="comfly"):
    ...
    # 兼容旧的 "comfly" 硬编码：?comfly 不存在或未指定，回退到首?provider
    ...
    if not provider:
        raise HTTPException(status_code=400, detail=f"未找?API 平台：{target}")
...
def get_api_provider_exact(provider_id: str):
    ...
    provider = next((p for p in providers if p["id"] == target), None)
    if not provider:
        raise HTTPException(status_code=400, detail=f"未找?API 平台：{target or '(empty)'}。新增平台未保存时请使用当前表单拉取模型?)
    if not provider.get("enabled", True):
        raise HTTPException(status_code=400, detail=f"API 平台已禁用：{provider.get('name') or target}")
    return provider
...
# 缓存 GitHub Tree API 响应（含 ETag），减少 60 ?h 限流压力
        ...
        with urllib.request.urlopen(req, timeout=30) as resp:
            ...
            if use_etag_cache and cache_key == GITHUB_TREE_URL:
                GITHUB_TREE_CACHE.update({
                    "etag": etag,
                    "data": payload,
                    "expires_at": time.time() + 600,  # 10 分钟内复?                })
            return payload
...
def update_from_github(req: UpdateRequest = UpdateRequest()):
    if not UPDATE_LOCK.acquire(blocking=False):
        raise HTTPException(status_code=409, detail="正在更新中，请稍后再?)
    try:
...
def rollback_update(req: RollbackRequest):
    ...
    try:
        ...
        if os.path.commonpath([backup_root_abs, backup_dir]) != backup_root_abs:
            raise HTTPException(status_code=400, detail="备份路径不安?)
        if not os.path.isdir(backup_dir):
            raise HTTPException(status_code=404, detail="备份不存?)
        restored = []
        skipped = []
...
class CanvasLLMRequest(BaseModel):
    ...
    provider: str = "comfly"
    ms_model: str = ""
    images: List[str] = []   # 可以?/output/*.png?assets/*.png 本地路径 ?http(s) URL ?data URL
...
def conversation_path(user_id, conversation_id):
    ...
    if not cleaned:
        raise HTTPException(status_code=400, detail="无效的对?ID")
...
def new_conversation(user_id, title="新对?):
    timestamp = now_ms()
    conversation = {
        "id": uuid.uuid4().hex,
        "title": (title or "新对?)[:80],
...
def canvas_path(canvas_id):
    ...
    if not cleaned:
        raise HTTPException(status_code=400, detail="无效的画?ID")
...
def new_canvas(title="未命名画?, icon="layers", kind="classic"):
    timestamp = now_ms()
    canvas_kind = normalize_canvas_kind(kind)
    canvas = {
        "id": uuid.uuid4().hex,
        "title": (title or ("智能画布" if canvas_kind == "smart" else "未命名画?))[:80],
...
def load_canvas(canvas_id):
    ...
    if not os.path.exists(path):
        raise HTTPException(status_code=404, detail="画布不存?)
    with open(path, 'r', encoding='utf-8') as f:
        canvas = json.load(f)
    if canvas.get("deleted_at"):
        raise HTTPException(status_code=404, detail="画布已在回收?)
    return canvas
...
def display_title(text):
    ...
    return title[:24] or "新对?

def resolve_chat_provider(provider: str, model: str, ms_model: str):
    if provider == "modelscope":
        if not MODELSCOPE_API_KEY:
            raise HTTPException(status_code=400, detail="未配?MODELSCOPE_API_KEY，请?API/.env 中填写?)
    ...
    if not base_root:
        raise HTTPException(status_code=400, detail=f"{api_provider.get('name') or api_provider['id']} 未配?Base URL")
...
def api_headers(json_body=True, provider=None):
    if provider:
        ...
        if not api_key:
            raise HTTPException(status_code=400, detail=f"未配?{provider_name} ?API Key，请?API 平台管理中填写?)
    else:
        api_key = AI_API_KEY
        if not api_key:
            raise HTTPException(status_code=400, detail="未配?COMFLY_API_KEY，请?API/.env 中填写?)
...
def modelscope_size(value, fallback="1024x1024"):
    ...
    if re.fullmatch(r"\d{2,5}x\d{2,5}", size):
        return size
    raise HTTPException(status_code=400, detail=f"ModelScope size 格式不正确：{value or fallback}，应?WxH，例?1024x1024")

def unwrap_apimart_response(raw):
    """APIMart 将标?OpenAI 响应包在 {"code":200,"data":{...}} 里；如果检测到就解包?""
    if isinstance(raw, dict) and "data" in raw and isinstance(raw.get("data"), dict) and "choices" not in raw:
        return raw["data"]
    return raw

def text_from_content_value(content, separator="\n"):
    if content is None:
        return ""
    ...
    if isinstance(content, list):
        parts = []
        for item in content:
            text = text_from_content_value(item, separator)
            if text:
                parts.append(text)
        return separator.join(parts)
    if isinstance(content, dict):
        for key in ("text", "content", "output_text", "value", "answer"):
            text = text_from_content_value(content.get(key), separator)
            if text:
                return text
        if isinstance(content.get("parts"), list):
            return text_from_content_value(content.get("parts"), separator)
        return ""
    return str(content)
...
def text_from_chat_response(data):
    ...
    if not isinstance(data, dict):
        return text_from_content_value(data)
    for key in ("output_text", "text", "answer", "content"):
        text = text_from_content_value(data.get(key))
        if text:
            return text
    reasoning_fallback = ""
        ...
        for choice in choices:
            ...
            if isinstance(message, dict):
                for key in ("content", "text"):
                    text = text_from_content_value(message.get(key))
                    if text:
                        return text
                if not reasoning_fallback:
                    reasoning_fallback = text_from_content_value(message.get("reasoning_content"))
            for key in ("text", "content"):
                text = text_from_content_value(choice.get(key))
                if text:
                    return text
    output = data.get("output")
    text = text_from_content_value(output)
    if text:
        return text
    ...
    for key in ("data", "result", "response"):
        ...
        if isinstance(nested, dict):
            text = text_from_chat_response(nested)
            if text:
                return text
...
def text_delta_from_chat_chunk(data):
    ...
    content = text_from_content_value(delta.get("content", ""), "")
    if content:
        return content
    return text_from_content_value(delta.get("text") or choices[0].get("text"), "")

def sse_event(data):
    return f"data: {json.dumps(data, ensure_ascii=False)}\n\n"

def image_data_from_string(value):
    text = str(value or "").strip()
    if not text:
        return None
    markdown = re.search(r"!\[[^\]]*\]\((data:image/[^\s)]+|https?://[^\s)]+)\)", text, re.I)
    if markdown:
        return image_data_from_string(markdown.group(1))
    data_url = re.search(r"data:image/[a-zA-Z0-9.+-]+;base64,[A-Za-z0-9+/=]+", text)
    if data_url:
        text = data_url.group(0)
    if text.startswith("data:image/") and ";base64," in text:
        ...
        return {"type": "b64", "value": encoded.strip(), "mime_type": mime_type}
...
def image_data_from_any(value):
    if value is None:
        return None
    if isinstance(value, str):
        return image_data_from_string(value)
    if isinstance(value, list):
        for item in value:
            found = image_data_from_any(item)
            if found:
                return found
        return None
    if not isinstance(value, dict):
        return None
    for key in ("url", "imageUrl", "image_url", "uri", "fileUri", "file_uri"):
        raw = value.get(key)
        if isinstance(raw, str) and raw.strip():
            found = image_data_from_string(raw)
            if found:
                return found
    for key in ("base64", "b64", "b64_json", "image", "data", "result", "output"):
        if key in value:
            found = image_data_from_any(value.get(key))
            if found:
                return found
    return image_data_from_string(json.dumps(value, ensure_ascii=False))

def extract_image_candidate(value, depth=0):
    ...
    if isinstance(value, list):
        for item in value:
            found = extract_image_candidate(item, depth + 1)
            if found:
                return found
        return None
    if not isinstance(value, dict):
        return None
    inline = value.get("inlineData") or value.get("inline_data") or {}
    if isinstance(inline, dict) and inline.get("data"):
        return {
            "type": "b64",
            "value": str(inline.get("data") or "").strip(),
            "mime_type": inline.get("mimeType") or inline.get("mime_type") or "image/png",
        }
    for key in ("mimeType", "mime_type"):
        if isinstance(value.get(key), str) and str(value.get(key)).startswith("image/") and value.get("data"):
            return {
                "type": "b64",
                "value": str(value.get("data") or "").strip(),
                "mime_type": value.get(key),
            }
    for key in ("b64_json", "base64", "b64"):
        if value.get(key):
            return {"type": "b64", "value": str(value[key]).strip(), "mime_type": value.get("mime_type") or "image/png"}
    for key in ("url", "imageUrl", "image_url", "uri", "fileUri"):
        raw = value.get(key)
        if isinstance(raw, str) and raw.strip():
            found = image_data_from_string(raw)
            if found:
                return found
    for key in ("image_url", "imageUrl", "url", "uri", "fileUri", "image", "images", "content", "parts", "message", "delta", "output", "result", "data", "choices", "candidates", "inlineData", "inline_data"):
        if key in value:
            found = extract_image_candidate(value.get(key), depth + 1)
            if found:
                return found
    return None

def extract_image(data):
    chat_image = extract_image_candidate(data)
    if chat_image:
        return chat_image
    if isinstance(data, dict):
        for key in ("image", "images", "outputs", "output", "result", "data"):
            candidate = data.get(key)
            if isinstance(candidate, list):
                for item in candidate:
                    found = extract_image_candidate(item)
                    if found:
                        return found
        ...
        for candidate in candidates:
            ...
            for part in parts:
                if not isinstance(part, dict):
                    continue
                found = extract_image_candidate(part)
                if found:
                    return found
                ...
                for key in ("url", "fileUri", "file_uri", "imageUrl", "image_url"):
                    raw = part.get(key)
                    if isinstance(raw, str) and raw.strip():
                        found = image_data_from_string(raw)
                        if found:
                            return found
    if isinstance(data, dict):
        for key in ("predictions", "artifacts", "generations", "choices"):
            candidate = data.get(key)
            if isinstance(candidate, list):
                for item in candidate:
                    found = extract_image_candidate(item)
...
def default_asset_library():
    return {
        "categories": [
            ...
            {"id": "workflows", "name": "工作?, "type": "workflow", "items": []},
        ],
        "updated_at": now_ms(),
    }
...
def load_asset_library():
    ...
    if not any(c.get("type") == "workflow" for c in cats):
        cats.append({"id": "workflows", "name": "工作?, "type": "workflow", "items": []})
    lib["categories"] = cats
...
def reference_to_data_url(ref, max_size=None):
    """把本地输出文件转?data URL（base64）。max_size 限制最长边像素，避?payload 过大?""
...
def apimart_upload_payload_from_bytes(data: bytes, mime: str, name_hint: str = "image"):
    """把内存中的图片字节按 APIMart ?10MB 限制压缩为可上传 payload?""
    max_bytes = 9_500_000
...
async def upload_image_for_apimart(client, provider, ref_url: str) -> str:
    """把本地图片转成上游可接受的输入?    ?APIMart 文档上传?/v1/uploads/images，拿到可用于生成接口?http/https URL?    绝不?/output/* ?/assets/* 这类本地路径直接传给上游?    返回上游可用 URL；返回值以 "ERR:" 开头表示具体失败原因（供前端展示）?""
    ref_url = str(ref_url or "").strip()
    if not ref_url:
        return "ERR:空地址"
    # 已经是网?URL ?asset:// ?直接可用，无需上传
    ...
    if ref_url.startswith("data:"):
        try:
            if ";base64," not in ref_url:
                return "ERR:不支持的 data URL（缺?base64 段）"
            ...
            if resp.status_code in (200, 201):
                ...
                return "ERR:APIMart 上传响应未包含可?URL"
            ...
            return f"ERR:上传异常 {e}"
    # 本地 /output/ ?/assets/ 路径：先确认文件存在再上?    if ref_url.startswith("/output/") or ref_url.startswith("/assets/"):
        ...
        try:
            ...
            return f"ERR:上传异常 {e}"
    return "ERR:不支持的图片来源（仅支持 http/https/asset/data 或本?/output/ /assets/ 路径?
...
async def generate_modelscope_provider_image(prompt, size, model, reference_images=None, provider=None):
    ...
    if not clean_token:
        raise HTTPException(status_code=400, detail="未配?ModelScope API Key，请?API 设置中填写?)
    width, height = parse_size_pair(size)
    refs = []
    for ref in (reference_images or [])[:4]:
        if not ref.get("url"):
            continue
        # 把参考图压缩?data URL，避?base64 payload 过大导致 MS 内部任务失败
    ...
    async with httpx.AsyncClient(timeout=AI_REQUEST_TIMEOUT) as client:
        ...
        if not task_id:
            try:
                ...
                raise HTTPException(status_code=502, detail=f"ModelScope 未返?task_id：{raw}")
...
async def generate_gemini_provider_image(prompt, size, model, reference_images=None, provider=None):
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(connect=20.0, read=1800.0, write=120.0, pool=20.0)) as client:
        if provider and not is_gemini_provider(provider):
            key_env = provider_key_env(provider["id"])
            api_key = os.getenv(key_env, "")
            if not api_key:
                raise HTTPException(status_code=400, detail=f"未配?{provider.get('name') or provider['id']} ?API Key，请?API 平台管理中填写?)
        ...
        raw = response.json()
        try:
            print(f"[gemini-image] response keys={list(raw.keys()) if isinstance(raw, dict) else type(raw).__name__} raw={json.dumps(raw, ensure_ascii=False)[:2000]}")
        except Exception as e:
            print(f"[gemini-image] log raw failed: {e}")
        return extract_image(raw), raw
...
def runninghub_api_headers(provider):
    ...
    if not api_key:
        raise HTTPException(status_code=400, detail="未配?RunningHub API Key，请?API 设置中填写?)
...
async def runninghub_upload_reference(client, provider, ref):
    ...
    for item in candidates:
        ...
        if value:
            return str(value)
    raise HTTPException(status_code=502, detail=f"RunningHub 上传图片未返?download_url：{raw}")
...
async def generate_runninghub_provider_image(prompt, size, model, reference_images=None, provider=None):
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(connect=20.0, read=1800.0, write=180.0, pool=20.0)) as client:
        ...
        try:
            ...
            if not task_id:
                raise HTTPException(status_code=502, detail=f"RunningHub 未返?taskId 或图片结果：{raw}")
...
async def generate_ai_image(prompt, size, quality, model, reference_images=None, provider_id="comfly"):
    ...
    if not base_url:
        raise HTTPException(status_code=400, detail=f"{provider.get('name') or provider['id']} 未配?Base URL")
    ...
    async with httpx.AsyncClient(timeout=request_timeout) as client:
        ...
        if is_apimart:
            ...
            # APIMart ?GPT-Image-2 图生图仍?/images/generations?            # 通过 image_urls 传参考图，不使用 OpenAI multipart /images/edits?            body = {
                "model": model,
                "prompt": prompt,
                "n": 1,
                "size": apimart_size,
            ...
            # 1) OpenAI 协议的图生图/编辑?multipart 提交?/images/edits?            # GPT-Image-2 参考图不能?/images/generations JSON，否则部分平台会忽略原图或报 Images API unsupported?            files = []
            opened = []
            ...
            try:
                ...
                for fh in opened:
                    fh.close()
            # 2) edits 失败 ??GPT-Image-2 可回退?/images/generations + JSON image:[urls/base64]（grsai 风格?            if response is None:
                if is_gpt2:
                    raise HTTPException(
                        status_code=502,
                        detail=f"GPT-Image-2 编辑接口 /images/edits 调用失败：{edit_failed_text[:300] or edit_failed_status}。已停止自动重试，避免上游可能已扣费后再次请求?
                    )
                print(f"/images/edits failed ({edit_failed_status}): {edit_failed_text[:200]} ?回退?/images/generations + image:[] JSON")
...
def view_image(filename: str, type: str = "input", subfolder: str = ""):
    # 先按原逻辑去各 ComfyUI 后端?    for addr in COMFYUI_INSTANCES:
        try:
    ...
    # 适用场景：画布通过 /api/ai/upload 把参考图直接落到本地 assets/input/?    # ?ComfyUI ?input 可能因为重启/清理而丢失，导致 enhance/klein 等页面预览对比图 404
...
async def save_providers(payload: List[ApiProviderPayload]):
    ...
    if not providers:
        raise HTTPException(status_code=400, detail="至少保留一?API 平台")
    # 强制最多一?primary（取最后被标记的；都没标记则保持原样不强制?    primary_indices = [i for i, flag in enumerate(raw_primary_flags) if flag]
    if primary_indices:
    ...
    if env_updates:
        ...
        reload_env_globals()   # 立即将最?env 值同步回模块全局变量，无需重启
    return {"providers": [public_provider(p) for p in providers]}

# --- ModelScope Token (?env 读取，不再支持通过 UI 修改) ---

@app.get("/api/config/token")
async def get_global_token():
    # 优先?env，回退?global_config.json（兼容旧数据?    if MODELSCOPE_API_KEY:
...
async def test_provider_connection(payload: TestConnectionPayload):
    ...
    if not re.match(r"^https?://", base_url):
        raise HTTPException(status_code=400, detail="请求地址必须?http:// ?https:// 开?)
    api_key = (payload.api_key or "").strip()
    if not api_key and payload.provider_id:
        api_key = os.getenv(provider_key_env(payload.provider_id), "")
    if not api_key:
        raise HTTPException(status_code=400, detail="请先填写或保?API Key")
...
async def probe_async_endpoint(payload: TestConnectionPayload):
    """验证异步协议：用?task_id 请求 GET /v1/tasks/{fake_id}?    收到 400 Invalid task ID = 端点存在?Key 有效?01/403 = Key 无效?04/连接失败 = 不支持异步端点?""
    base_url = (payload.base_url or "").strip().rstrip("/")
    if not base_url:
        raise HTTPException(status_code=400, detail="请先填写请求地址")
    api_key = (payload.api_key or "").strip()
    ...
    try:
        ...
        # 404 + 没有结构化错??平台不支持此端点
        if sc == 404:
            return {"ok": False, "status_code": sc, "message": "平台不支?/v1/tasks/ 端点，可能不?APIMart 异步协议", "raw": body}
        # 其他 400 ??返回原始信息供参?        if 400 <= sc < 500:
        ...
        return {"ok": False, "status_code": sc, "message": f"服务端错?{sc}", "raw": body}
    except httpx.HTTPError as e:
...
async def fetch_upstream_models(provider_id: str):
    """从已保存的上?OpenAI 兼容接口拉取 /v1/models 列表，按名称智能分类?image/chat/video?""
    provider = get_api_provider_exact(provider_id)
    api_key = os.getenv(provider_key_env(provider["id"]), "")
    if not api_key:
        raise HTTPException(status_code=400, detail=f"{provider.get('name') or provider_id} 未配?API Key")
...
async def build_online_image_result(payload: OnlineImageRequest):
    ...
    try:
        ...
        # 把上游英文错误转成中文友好提?        friendly = None
        m = re.search(r"longest edge must be less than or equal to (\d+)", text)
        if m:
            limit = m.group(1)
            friendly = f"该模型不支持当前分辨率：最长边超过 {limit}px。请把图片分辨率调低（例如换?2K 或更小），或更换支持高分辨率的模型?
            ...
            friendly = "API Key 无效或已过期，请到「API 设置」检?Key?
...
async def get_canvas_image_task(task_id: str):
    ...
    if not task:
        raise HTTPException(status_code=404, detail="画布任务不存在，可能服务已重启或任务已过?)
    return task
...
async def wait_for_video_task(client, provider, task_id):
    ...
    if is_apimart_provider(provider):
        ...
        task_url = f"{base_url}/v1/videos/{task_id}"
    ...
    while time.monotonic() < deadline:
        ...
        if status in VIDEO_TASK_SUCCESS_STATUSES:
            return raw
        # 部分上游不返回标?status 字段，但已经返回了视?URL —?直接当成功处?        if not status and video_output_urls(raw):
            return raw
...
async def canvas_video(payload: CanvasVideoRequest):
    ...
    api_key = os.getenv(provider_key_env(provider["id"]), "")
    if not api_key:
        raise HTTPException(status_code=400, detail=f"未配?{provider.get('name') or provider['id']} ?API Key，请?API 设置中填写?)
    is_apimart = is_apimart_provider(provider)
    submit_url = f"{base_url}/videos/generations" if is_apimart and base_url.endswith("/v1") else f"{base_url}/v1/videos/generations" if is_apimart else f"{base_url}/v1/videos"
    ...
    try:
        async with httpx.AsyncClient(timeout=VIDEO_POLL_TIMEOUT) as client:
            # --- 构造图片载?---
            if is_apimart:
                # APIMart 只接?http/https ?asset:// URL，先上传本地图片取回网络 URL
                image_with_roles = []
                invalid_images = []  # 每项?(原始 URL, 失败原因)
                apimart_model = apimart_veo31_model(requested_model) if is_veo31 else ""
                if apimart_model == "veo3.1-lite" and payload.images:
                    raise HTTPException(status_code=400, detail="veo3.1-lite 不支持图片输入，请改?veo3.1-fast ?veo3.1-quality?)
                ...
                # ?APIMart：data URL 方式（OpenAI / ComflyAI 接口?                image_payload = []
            ...
            try:
                ...
                # 上游返回?HTML 错误页面或非 JSON 响应
                resp_text = response.text[:500]
                raise HTTPException(status_code=502, detail=f"上游视频接口返回?JSON 响应（状?{response.status_code}）：{resp_text}")
        ...
        if "channel not found" in text or "model_not_found" in text:
            hint = (
                f"上游「{provider_name}」识别了模型「{requested_model}」，但你?API Key 账号?*没有该模型的可用通道**。\n\n"
                f"原因：你的账号没开通这个模型的访问权限（付?订阅相关）。\n\n"
                f"解决方法：\n"
...
async def canvas_llm(payload: CanvasLLMRequest):
    ...
    # 构造用户消息：有图片时?OpenAI vision 多模态格?    image_inputs = [img for img in (payload.images or []) if is_image_reference_value(img)]
    if image_inputs:
        ...
        for img in image_inputs[:8]:
            if not img or not isinstance(img, str):
                continue
            # 本地 /output/* ?/assets/* 路径转为 data URL；http(s) ?data URL 直接?            if img.startswith("/output/") or img.startswith("/assets/"):
    ...
    try:
        async with httpx.AsyncClient(timeout=AI_REQUEST_TIMEOUT) as client:
            req_body = {"model": model, "messages": upstream_messages, "stream": False}
            response = await client.post(
...
async def rename_asset_library_category(category_id: str, payload: AssetLibraryRenameRequest):
    ...
    if not cat:
        raise HTTPException(status_code=404, detail="分类不存?)
...
async def delete_asset_library_category(category_id: str):
    ...
    if cat.get("type") == "workflow" and category_id == "workflows":
        raise HTTPException(status_code=400, detail="默认工作流分类不能删?)
...
async def add_asset_library_item(payload: AssetLibraryAddRequest):
    ...
    if cat.get("type") != "image":
        raise HTTPException(status_code=400, detail="该分类暂不支持添加图?)
    src = output_file_from_url(payload.url)
    if not src:
        raise HTTPException(status_code=400, detail="只支持保存本?/assets ?/output 图片")
    ...
    for cat in lib.get("categories", []):
        for item in cat.get("items", []):
            if item.get("id") == item_id:
                ...
                save_asset_library(lib)
                return {"library": lib, "item": item}
    raise HTTPException(status_code=404, detail="资产不存?)
...
async def update_canvas(canvas_id: str, payload: CanvasSaveRequest):
    ...
    canvas["title"] = (payload.title or canvas.get("title") or "未命名画?)[:80]
...
async def chat(payload: ChatRequest, request: Request, x_user_id: str = Header(default="")):
    ...
    if payload.mode == "image":
        ...
        try:
            async with httpx.AsyncClient(timeout=AI_REQUEST_TIMEOUT) as client:
                conv_req_body = {"model": model, "messages": upstream_messages, "stream": False}
                response = await client.post(
                    f"{chat_base}/chat/completions",
                    headers=chat_hdrs,
                    json=conv_req_body,
...
async def chat_stream(payload: ChatRequest, request: Request, x_user_id: str = Header(default="")):
    if payload.mode == "image":
        raise HTTPException(status_code=400, detail="图片模式请使?/api/chat")
...
async def poll_angle_cloud(req: CloudPollRequest):
    ...
    if not clean_token:
        raise HTTPException(status_code=400, detail="未提?ModelScope API Key")
...
async def ms_generate(req: MsGenerateRequest):
    ...
    clean_token = (req.api_key or MODELSCOPE_API_KEY).strip()
    if not clean_token:
        raise HTTPException(status_code=400, detail="未配?ModelScope API Key，请?API 设置中填写，或重新保?ModelScope Token?)
...
LEGACY_CUSTOM_WORKFLOW_FOLDER = "自定?
...
def save_comfyui_instances(payload: ComfyInstancesPayload):
    # 宽容校验：去前后空白、去 http(s):// 前缀、去尾部斜杠；要求形?host:port
    cleaned = []
    for item in payload.instances:
        ...
        s = s.rstrip("/")
        if ":" not in s:
            raise HTTPException(status_code=400, detail=f"地址缺少端口号：{item}（应?host:port，例?127.0.0.1:8188?)
        host, _, port = s.rpartition(":")
        if not host or not port.isdigit():
            raise HTTPException(status_code=400, detail=f"地址不合法：{item}（应?host:port，例?127.0.0.1:8188?)
        if s in cleaned:
            continue
        cleaned.append(s)
    if not cleaned:
        raise HTTPException(status_code=400, detail="至少保留一?ComfyUI 后端地址")
    # 写入 env 文件
    try:
...
def upload_workflow(payload: WorkflowUploadRequest):
    ...
    if not isinstance(payload.workflow, dict) or not payload.workflow:
        raise HTTPException(status_code=400, detail="工作?JSON 为空")
    # 简单校验：?API 格式（节?id ?key，含 class_type?    sample = next(iter(payload.workflow.values()), None)
    if not isinstance(sample, dict) or "class_type" not in sample:
        raise HTTPException(status_code=400, detail="不是有效?ComfyUI API 工作?JSON（需包含 class_type?)
...
def delete_workflow(name: str):
    ...
    if is_builtin_workflow(name):
        raise HTTPException(status_code=400, detail="内置工作流不可删?)
    workflow_path = workflow_path_from_name(name)
    cfg_path = workflow_config_path(name)
    if not os.path.exists(workflow_path):
        raise HTTPException(status_code=404, detail="Workflow not found")
    ...
    for field in payload.config.fields:
        ...
        if field.id in payload.fields:
            ...
            if field.type in ("number", "slider"):
                ...
                # 下拉值如果看起来是数字（?"1024" / "2048" / "0.8"），自动转成 int/float
                if isinstance(value, str):
                    s = value.strip()
                    try:
